...
class BinaryDiceLoss(nn.Module):
	...
	def forward(self, pred, label):
		...
		smooth = 1  # smooth variable (in case of denominator = 0)
...
class BinaryCrossEntropyLoss(nn.Module):
	...
	def forward(self, pred, label):
		...
		loss_combine = - (label_flat * log_pred + one_minus_label * log_one_minus_pred)
...
class DiceCELoss(nn.Module):
	...
	def forward(self, pred, label):
		loss = self.dice(pred, label) + self.bce(self.bce)
		return loss
